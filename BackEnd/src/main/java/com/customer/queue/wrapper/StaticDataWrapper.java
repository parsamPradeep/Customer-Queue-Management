...
public class StaticDataWrapper {
	...
	public ResponseModel getAllServiceTypes(RequestModel requestModel) throws Exception {
		...
		if (!allServiceTypeData.isEmpty())
			return this.customerQueueServiceUtility.generateSuccessResponse(allServiceTypeData,
					SuccessCodes.S_SERVICE_TYPES_OBTAINED.toString());
		else
			return this.customerQueueServiceUtility.generateSuccessResponse(requestModel,
					SuccessCodes.S_NO_SERVICE_TYPE_DATA.toString());

	}

	public ResponseModel getServiceTypesForBranch(RequestModel requestModel) throws Exception {
		log.debug("i am in the com.customer.queue.wrapper to getServiceTypesForBranch");
		log.debug(requestModel.toString());
		try {
			ServiceType serviceType = customerQueueServiceUtility.validateForGetServiceTypeForBranch(requestModel);
			List<ServiceType> serviceTypesForBranch = StaticDataService.getServiceTypesForBranch(serviceType);
			if (!serviceTypesForBranch.isEmpty())
				return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypesForBranch,
						SuccessCodes.S_SERVICE_TYPES_OBTAINED.toString());
			else
				return this.customerQueueServiceUtility.generateSuccessResponse(requestModel,
						SuccessCodes.S_NO_SERVICE_TYPE_DATA.toString());
	...
	public ResponseModel removeServiceType(RequestModel requestModel) {
		...
		log.debug(requestModel.toString());
		try {
			ServiceType serviceType = customerQueueServiceUtility.validateForRemoveServiceType(requestModel);
			List<ServiceType> deletedServiceTypeData = StaticDataService.removeServiceType(serviceType);
			requestModel.setData(deletedServiceTypeData);
			return this.customerQueueServiceUtility.generateSuccessResponse(requestModel,
					SuccessCodes.S_SERVICE_TYPE_REMOVED.toString());
	...
	public ResponseModel addServiceType(RequestModel requestModel) {
		...
		try {
			...
			return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypeDataUponAddition,
					SuccessCodes.S_SERVICE_TYPE_ADDED.toString());
	...
	public ResponseModel getAllCounters(RequestModel requestModel) throws Exception {
		...
		if (!counterData.isEmpty())
			return this.customerQueueServiceUtility.generateSuccessResponse(counterData,
					SuccessCodes.S_COUNTERS_OBTAINED.toString());
		else
			return this.customerQueueServiceUtility.generateSuccessResponse(requestModel,
					SuccessCodes.S_NO_COUNTERS_DATA.toString());
	...
	public ResponseModel removeCounter(RequestModel requestModel) {
		...
		try {
			...
			return this.customerQueueServiceUtility.generateSuccessResponse(counterData,
					SuccessCodes.S_COUNTERS_REMOVED.toString());
	...
	public ResponseModel addCounter(RequestModel requestModel) {
		...
		try {
			...
			return this.customerQueueServiceUtility.generateSuccessResponse(counterData,
					SuccessCodes.S_COUNTER_ADDED.toString());
	...
	public ResponseModel addServiceTypeForCounter(RequestModel requestModel) {
		...
		try {
			...
			return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypeForCounter,
					SuccessCodes.S_SERVICE_TYPE_FOR_COUNTER_ADDED.toString());
	...
	public ResponseModel removeServiceTypeForCounter(RequestModel requestModel) {
		...
		try {
			...
			return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypeForCounter,
					SuccessCodes.S_SERVICE_TYPE_FOR_COUNTER_REMOVED.toString());
	...
	public ResponseModel getAllServiceTypeForCounter(RequestModel requestModel) throws Exception {
		...
		if (!serviceTypeForCounterData.isEmpty())
			return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypeForCounterData,
					SuccessCodes.S_SERVICE_TYPE_FOR_COUNTER_OBTAINED.toString());
		else
			return this.customerQueueServiceUtility.generateSuccessResponse(requestModel,
					SuccessCodes.S_NO_SERVICE_TYPE_FOR_COUNTER_DATA.toString());
	}
	...
	public ResponseModel getServiceTypeForCounter(RequestModel requestModel) throws Exception {
		...
		log.debug(requestModel.toString());
		ServiceTypeForCounter serviceTypeForCounter = customerQueueServiceUtility
				.validateForgetServiceTypeForCounter(requestModel);
		List<ServiceTypeForCounter> serviceTypeForCounterData = StaticDataService
				.getServiceTypeForCounter(serviceTypeForCounter);
		if (!serviceTypeForCounterData.isEmpty())
			return this.customerQueueServiceUtility.generateSuccessResponse(serviceTypeForCounterData,
					SuccessCodes.S_SERVICE_TYPE_FOR_COUNTER_OBTAINED.toString());
		else
